...
export function paginationInit(
  ...
  function updateUI() {
    const formattedCurrent = currentPage.toString().padStart(1, '0');
    const formattedTotal = totalPages.toString().padStart(1, '0');

    if (currentCountEl) currentCountEl.value = formattedCurrent;

    const maxSpan = paginationWrapper.querySelector('[data-planning-pages-count]');
    if (maxSpan) maxSpan.textContent = formattedTotal;
